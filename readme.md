...
`./third_party/BundleTrack/XMem/saves/XMem-s012.pth`
...
`./third_party/BundleTrack/LoFTR/weights/outdoor_ds.ckpt`

- Download HO3D data. We provide the augmented data that you can download [here](https://drive.google.com/drive/folders/1Wk-HZDvUExyUrRn7us4WWEbHnnFHgOAX?usp=share_link). Then download YCB-Video object models from [here](https://drive.google.com/file/d/1-1m7qMMyUHYLhaRiQBbsSRMt5dMRX4jD/view?usp=share_link). Finally, make sure the structure is like below, and update your root path of `HO3D_ROOT` at the top of `./third_party/BundleTrack/scripts/data_reader.py`
...
- Instantiate and start-up a docker container of the image

```
docker compose up -d

# Or from locations other than the docker dir
...
```
docker exec -it bundlesdf-blackwell bash
```

- Build the env after getting inside the container

```
bash build.sh
```

- (Optional) In case of setting up the container on the GPU server of 2025-11-01 or later, you may need to run the following command additionally (cf. <https://github.com/NVlabs/BundleSDF/issues/197>)
...
python3 run_custom.py --mode draw_pose --out_folder ./results/milk
