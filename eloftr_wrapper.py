...
"""
EfficientLoFTR wrapper for BundleSDF.
Based on loftr_wrapper.py but using EfficientLoFTR for faster inference.
"""
...
sys.path.insert(0, f'{code_dir}/third_party/BundleTrack/EfficientLoFTR')
...
class ELoftrRunner:
  """EfficientLoFTR-based feature matcher for BundleSDF."""
  
  def __init__(self):
    _default_cfg = deepcopy(full_default_cfg)
    print("EfficientLoFTR config:", _default_cfg)
    
    self.matcher = LoFTR(config=_default_cfg)
    self.matcher.load_state_dict(
        torch.load(f'{code_dir}/third_party/BundleTrack/EfficientLoFTR/weights/eloftr_outdoor.ckpt',
                   weights_only=False)['state_dict'])
    self.matcher = reparameter(self.matcher)  # Essential for EfficientLoFTR performance
  ...
  def predict(self, rgbAs:np.ndarray, rgbBs:np.ndarray):
    '''
    Match feature points between image pairs.
    
    @rgbAs: (N,H,W,C) - batch of reference images
