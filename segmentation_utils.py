...
class Segmenter():
    ...
    def _load_sam3(self):
        if self.sam3 is None:
            ...
            try:
                ...
                sam3_pkg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'third_party', 'sam3')
    ...
    def get_first_frame_mask(self, image_path):
        ...
        print(f"Please view this image files on your host machine to determine the prompt.")
        print("Enter a text prompt to segment the object (e.g., 'milk carton', 'hand', 'cat')")
        print("="*50 + "\n")
        ...
        print(f"Debug: Image shape: {vis_image.shape}, Combined mask shape (raw): {combined_mask.shape}")
        ...
        if combined_mask.shape[:2] != vis_image.shape[:2]:
            print(f"Warning: Mask shape {combined_mask.shape} does not match image shape {vis_image.shape[:2]}. Resizing mask.")
            combined_mask = cv2.resize(combined_mask, (vis_image.shape[1], vis_image.shape[0]), interpolation=cv2.INTER_NEAREST)
        ...
        if combined_mask.max() > 0:
            ...
            vis_image[mask_bool] = cv2.addWeighted(vis_image[mask_bool], 0.5, red_mask[mask_bool], 0.5, 0).reshape(-1, 3)
