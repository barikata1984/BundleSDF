...
- [ ] gridencoder の `ModuleNotFoundError` の恒久対処 (2026-07-03 発見, NeRF を回す全実行に影響, 次回セッション最優先). `mycuda/setup.py` は `gridencoder` をトップレベル拡張としてビルドし `.so` を `/workspace/mycuda/gridencoder.cpython-310-*.so` に配置するが, `mycuda/torch_ngp_grid_encoder/grid.py:23` は自ディレクトリのみを `sys.path` に足して bare `import gridencoder` するため, `multiprocessing.Process` で spawn される NeRF ワーカー (sys.path は repo root のみ) から解決できずクラッシュする. 今回は config・コードを変更せず `PYTHONPATH=/workspace/mycuda` で回避したのみ. `grid.py` が親ディレクトリを `sys.path` に足すか, `gridencoder` を import 可能な場所に install するかたちで恒久対処する
- [ ] VRAM 計測ロジック (`perf_logger.py`/`bench_milk.sh`) がフルベンチ実行時に実際に `perf_main.csv` へ `vram_alloc_mib`/`vram_reserved_mib` 列を出力することの動作確認 (smoke test のみ確認済み, フルベンチでの動作確認は未実施)
