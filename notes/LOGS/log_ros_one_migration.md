...
## 2026-07-06: delay-精度トレードオフの最終スイープ実施と, 実行中の2件のインシデント

`notes/PLAN_delay_accuracy_sweep.md` (2026-07-03 に方針転換され, 「他の改善項目を全部実装し終えた最終構成で1回のスイープにまとめる」こととした計画) を受け, `notes/PERF_plan.md` の改善項目 (マッチャーの opt 設定切替・warm-start・MPS 恒久化) が出そろった現行既定構成で, HO3D の delay-精度トレードオフの網羅的スイープを実施した.

### スイープの構成と結果

対象は HO3D 評価セット13動画 (AP10-14, MPM10-14, SB11, SB13, SM1) で, `sync_max_delay` を 3, 6, 12, 24 の4点で振り, 各動画3セット (再現性確認用) を実行した (計156セル). 第1〜2段階で使った 3,4,5,6,7,8,10,15 という細かい delay 刻みは, 13動画 × 3セットの規模でコストが増大するため 4 点に簡略化した.

154/156セルが完了した. MPM13 を除く12動画の平均で, delay 3/6/12/24 に対し ADD 誤差 1.139→1.198→1.219→1.256cm (+10%悪化), 壁時計 326.8→252.3→235.2→233.8秒 (-28%短縮), fps 換算 4.80→6.21→6.67→6.70 だった. delay=6 以降は速度向上がほぼ頭打ちになる一方, 精度悪化は delay=24 までほぼ線形に続いた. 追加で delay=48 を1セット (MPM13 除く12動画) 実行し, delay=24 からの頭打ちが継続することを確認した (平均 ADD 1.177cm, 壁時計229秒). 唯一の例外は MPM10 で, delay=3→24 の単調悪化 (1.17→2.45cm) から delay=48 で 1.08cm へ反転した (n=1 であり, 再現性は未確認).

結論として, delay=6〜12 あたりが速度と精度のバランス点である. delay=24 以降まで上げる追加の速度メリットはほぼ無いが, 精度コストだけが乗り続ける.

第1〜2段階の3動画24回分のスイープ (`data/bench_results/ho3d_sweep/`, matcher opt 導入前の設定) は条件が異なるため上書きせず, 参考データとして残置した. 新データは `data/bench_results/ho3d_sweep_opt/` (set1/set2/set3ディレクトリ) に保存し, 生ログ (`sweep_progress.log`, 全166セル分・delay=48込み) とクラッシュ記録 (`sweep_errors.log`) も同ディレクトリに置いた. delay-精度の全データ・グラフ・インシデント記録をまとめた HTML レポートを `data/bench_results/ho3d_sweep_opt/report.html` に作成した.

### MPM13 の恒久的な破綻

MPM13 (`potted_meat_can` グループの1本) は全セット・全 delay で破綻した (ADD 9.2〜11.4cm. 他動画は概ね0.5〜3.0cm. forgetting frame ~2900回/1618フレーム中). frame1250〜1290帯で計15回中12回「NeRF worker process died」でクラッシュし (Python トレースバックなし), クラッシュ時のキーフレーム数が一貫して102〜113という狭い範囲に集中していた (フレーム番号自体はばらつく). この集中は, フレーム固有のデータ破損ではなく, キーフレーム集合がこの規模に達したときの固定サイズバッファ境界か数値不安定性が原因である可能性を示唆するが, 根本原因は未特定のまま残した. set2 の delay=6,12 は3回再試行しても再現し, 恒久欠損として確定した (156-欠損4=152セルに, 後の再試行で2セル復旧し154セル). `notes/ISSUES.md` に新規課題として追加した.

### インシデント1: 9時間の無言クラッシュ (未解決)

set1 実行中, バックグラウンド driver スクリプト方式で AP13 delay=12 を実行していたプロセスツリーが, Python トレースバック・CUDA OOM メッセージなど一切の痕跡を残さず消失し, 9時間気づかれなかった.

原因調査では, OOM killer (cgroup `memory.events` で確認), GPU OOM, GPU ECC/Xid エラー, ディスク枯渇, セマフォリーク, timeout による強制終了, コンテナ/ホストの再起動, 人為的操作, 他セッションの影響のいずれも証拠上否定された. しかしコンテナの cgroup namespace 分離によりホスト側の状況を直接見る手段がなく, 根本原因は特定できないまま, これ以上の追跡は不可能と判断した.

対策として, 実行方式をバックグラウンド放置から「1セルずつ foreground・timeout 付き Bash で同期実行し, エージェント自身が完了を直接見届ける」方式に変更した. 以降, 同種の無言クラッシュは再発していない. `notes/ISSUES.md` に未解決課題として追加した.

### インシデント2: 無関係な ROS ノードの誤 kill

set2 実行中, MPM10 delay=3 の CUDA クラッシュ (illegal memory access) に対応していた際, GPU 上で動いていた別プロセス (PID 304885, `bundlesdf_node.py`) を自ジョブの孤立プロセスと誤認して kill してしまった.

実際は別セッション (またはユーザー本人) が実行していた実機検証だった. `notes/TODO.md` の「保留中」セクションに記載のある「実カメラハードウェアでの検証」に向けた前段作業として, モックカメラストリーム (2026-07-05 実装済み, 本ログ参照) を使った `bundlesdf_node` のパイプライン検証をユーザー側 (または並行セッション) が行っていたものであり, ユーザー側で対応済みである.

当初「GPU 競合が MPM10 のクラッシュ原因ではないか」と推測したが, 後の検証でそのノードは終始「点群0点で FAIL」という軽い判定を繰り返すのみで重い計算に入っておらず, 競合説は根拠が弱いと判明したため撤回した. MPM10 クラッシュの真因も結局特定できていない.
